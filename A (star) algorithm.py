...
def A_star(map, start, end):
    ...
    Dist_from_end=np.full((h, w), np.inf) # Initially, all points are at infinity wrt end
    Dist_from_start[start] = 0
    Dist_from_end[start]=calcDist(start,end)
    Dist_from_start[end] = 0
            ...
            for j in range(-1, 2):
                ...
                if inImg(img, point):
                    if (map[point] == WHITE).all():
                        if (calcDist(point, current) + Dist_from_start[current]+calcDist(point, current) + Dist_from_end[current] < Dist_from_start[point]+Dist_from_end[point]):

                            Dist_from_start[point] = calcDist(point, current) + Dist_from_start[current]
                            parent[point[0], point[1]] = [current[0], current[1]]
                            Dist_from_end[point] = calcDist(point, end) #Euclidean distance


        min_dist = np.inf
        # This finds out among all points whose distances has been assigned, which is closest to start

        hypothetical = (Dist_from_start+Dist_from_end).copy()
